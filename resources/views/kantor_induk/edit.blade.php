...
<hr>
<form class="user" action="{{ route('kantorinduk.update',$kantorinduk->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="form-group">

            <input value="{{ $kantorinduk->kode_induk }}" name="kode_induk" type="text" class="form-control form-control-user @error('kode_induk') is-invalid @enderror" id="Name" placeholder="Full Name">
            @error('kode_induk')
            <span class="invalid-feedback" role="alert">{{ $message }}</span>
            @enderror
    </div>
    <div class="form-group">
        <input type="text" value="{{ $kantorinduk->nama_induk }}" name="nama_induk" class="form-control form-control-user @error('nama_induk') is-invalid @enderror" id="exampleInputEmail"
